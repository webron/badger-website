...
# without failing.
#
# store-testflight is retired but kept: it is what the iOS affordance was called
# before the App Store listing existed, and dropping it here would silently hide
# the taps already recorded under that name.
STORE_EVENTS = {
...
def main() -> None:
    ...
    if not any(events.get(path) for path in STORE_EVENTS):
        ...
        for path, label in STORE_EVENTS.items():
            ...
            if count == 0 and "retired" in label:
                continue
